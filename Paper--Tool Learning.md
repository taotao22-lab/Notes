...
- **在数据合成方面的主要观察**：
    ...
    1. 关于数据合成方面说的不是很详细，只有这一段：
        ...
        1. 收集了一系列**智能体框架**、**真实世界的工具API** 和 **MCP 服务器**，并利用大语言模型**构建和模拟**一批工具（感觉把能用到的方式都用到了，但是不知道数量是多少）
    
        2. 生成人物，包括单步骤和多步骤的工具调用场景 （query应该也是合成的）
    
        3. 利用LLM生成工具调用轨迹。此外，LLM也作为用户模拟器

        4. 对于每个生成的轨迹，我们使用多个模型来判断任务是否完成。仅保留成功的轨迹
